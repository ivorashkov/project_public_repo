...
export const Dashboard = () => {
  ...
  const onSubmitHandler = async (e:FormEvent<HTMLFormElement>) => {
    ...
    await fetch(`http://localhost:8091/api/user/upload/all/${data?.userData?.id}`, {
        mode: 'no-cors',
        method: 'POST',
        headers: {
  ...
  const inactiveSections = <>
    <div className="note">
      <p>
        plis upload files u asshole, {data?.userData?.sub}! 
      </p>
    </div>
    ...
    <div className="form">
      <form action="" onSubmit={onSubmitHandler} encType="multipart/form-data">
        <div className="form__row">
          ...
          <input type="file" name="file" id="file" onChange={(event) => setFiles(event.target.files)} multiple required/>
        </div>
